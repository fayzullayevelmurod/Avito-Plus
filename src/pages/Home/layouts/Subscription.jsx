    ...
    <Box py="80px">
      <Flex justify="space-around" gap="20px" align="center">
        <Heading fontSize="30px" fontWeight="800" textAlign="center">
          Доступные тарифы <br />
          <Box color="#805AD5" as="span">Для бизнеса</Box>
        </Heading>
      ...
      {plans.map(plan => (
        <Fragment key={plan.id}>
          <Box my="25px" as="hr" />
          <Flex justify="space-around" gap="20px" align="center">
            <Heading textAlign="center" fontSize="20px" fontWeight={700}>{plan.title}</Heading>

            <Flex gap="14px" direction="column">
              {plan.info.map((planText, idx) => (
                <Flex gap="10px" align="center" key={idx}>
                  <Flex justify="center" align="center" as="span" bg="#35A169" w="20px" h="20px" borderRadius="100%">
                    <CheckIcon color="white" boxSize={3} />
                  </Flex>
                  <Text fontSize="14px" fontWeight={400}>{planText}</Text>
                </Flex>
              ))}
            </Flex>
